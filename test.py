...
def parse_args():
    ...
    parser.add_argument('--name', default='DR',
...
def main():
    ...
    test_dir = preprocess(
        ...
        pad=args.padding,
        remove=args.remove)
    test_df = pd.read_csv('./data/dataset/test/label.csv')
        ...
        with torch.no_grad():
            for i, (input, _) in tqdm(enumerate(test_loader), total=len(test_loader)):
                if test_args.tta:
                    ...
                    preds_fold.extend(output.data.cpu().max(1)[1].numpy())
    ...
    # confusion('submissions/%s.csv' %args.name, './data/dataset/test/label.csv')
